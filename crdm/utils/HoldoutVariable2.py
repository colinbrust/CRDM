...
def get_pred_true_arrays(model, mod_f, target, in_features, init, cuda, continuous, var_type, idx):
    # Get hyperparameters from filename
    info = parse_fname(mod_f)
    batch, nWeeks = int(info['batch']), int(info['nMonths'])
    data = AggregateAllPixles(target=target, lead_time=2,  in_features=in_features, n_weeks=nWeeks, init=init)
...
def save_all_preds(target_dir, in_features, mod_f, out_dir, remove, init, cuda, continuous):
    ...
    for f in targets:
        print(f)
        out = get_pred_true_arrays(model, mod_f, f, in_features, True, True, True, 'full', None)
        save_arrays(out_dir, out, f, True, 'full')
        
        for idx, var in enumerate(weeklys):
            print('{} - Holding out {}'.format(f, var))
            out = get_pred_true_arrays(model, mod_f, f, in_features, True, True, True, 'weekly', idx)
            save_arrays(out_dir, out, f, True, var)

        for idx, var in enumerate(MONTHLY_VARS):
            print('{} - Holding out {}'.format(f, var))
            out = get_pred_true_arrays(model, mod_f, f, in_features, True, True, True, 'monthly', idx)
            save_arrays(out_dir, out, f, True, var)
...
if __name__ == '__main__':
    ...
    save_all_preds(mod_f=args.model_file, target_dir=args.target_dir, in_features=args.in_features,
                   out_dir=args.out_dir, remove=True, init=True, cuda=cuda, continuous=True)
